...
def main():
    try:
        if sys.argv[1] not in "bw":
            print("Enter color of choice: either 'b' or 'w'")
            return 1
    except IndexError:
        print("Enter color of choice: either 'b' or 'w'")
    else:
...
class GameofChess(arcade.Window):
    ...
    def create_pieces(self):
        ...
        pieces = {}
        os.chdir("C:/Users/marek/Downloads/Python Projects/chess_game/chess_pieces")
        for color in "bw":
            ...
            for i in range(len(names)):
                ...
                sprite = arcade.Sprite(f"{name}_{color}.png")
    ...
    def run_engine(self):
        engine = chess.engine.SimpleEngine.popen_uci("C:/Users/marek/Downloads/Python Projects/chess_game/stockfish.exe")
        ...
        if len(move) == 5:
            if move[4] == "q":
                ...
                self.promtion_choice = "rook"
            elif move[4] == "n":
                self.promotion_choice = "knight"
            else:
                self.promotion_choice = "bishop"
        ...
        if button == arcade.MOUSE_BUTTON_LEFT and self.current_piece != None:
            ...
            if self.current_piece.name == "pawn" and (self.end[1] == "1" or self.end[1] == "8"):
                if self.promotion_choice == None:
                    self.promotion_choice = "queen"
                    move += "q"
                elif self.promotion_choice == "knight":
                    move += "n"
                else:
                    move += self.promotion_choice[0]
    ...
    def on_key_press(self, key, modifiers):
        if self.current_piece.name == "pawn" and (self.start[1] == "2" or self.start[1] == "7"):
            ...
            if key in keys:
                self.promotion_choice = keys[key]

    def make_move(self, move):
        if len(move) == 5:
            self.piece_sprites.remove(self.current_piece.sprite)
            sprite = arcade.Sprite(f"{self.promotion_choice}_{self.current_piece.color}.png")
